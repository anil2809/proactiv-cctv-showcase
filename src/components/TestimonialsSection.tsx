        ...
        <div className="max-w-4xl mx-auto">
          <div className="relative h-[450px] overflow-hidden">
                ...
                <div className="bg-white/5 dark:backdrop-blur-md backdrop-blur-lg border dark:border-white/10 border-gray-200 rounded-xl p-8 w-full max-w-2xl relative dark:bg-black/10 bg-white/80">
                  ...
                  <div className="flex flex-col md:flex-row items-center gap-6 mb-8">
                    <div className="relative w-20 h-20 rounded-full overflow-hidden border-2 border-primary/30 flex-shrink-0">
                      ...
                      <div className="w-20 h-20 rounded-full bg-primary/20 flex items-center justify-center text-2xl font-bold dark:text-white text-gray-800">
                        {testimonials[currentIndex].name.charAt(0)}
                      </div>
                    </div>
                    
                    <div className="text-center md:text-left">
                      <h4 className="text-xl font-semibold dark:text-white text-gray-900">{testimonials[currentIndex].name}</h4>
                      <p className="text-sm dark:text-white/60 text-gray-600">{testimonials[currentIndex].role}</p>
                    </div>
                  </div>
                  
                  <blockquote className="text-lg dark:text-white/90 text-gray-700 text-center md:text-left italic mb-6">
            ...
            <button
              ...
              className="absolute left-0 top-1/2 -translate-y-1/2 dark:bg-background/50 bg-white/70 backdrop-blur-sm border dark:border-white/10 border-gray-200 rounded-r-full p-3 hover:bg-primary/20 transition-all"
            ...
            <button
              onClick={handleNext}
              className="absolute right-0 top-1/2 -translate-y-1/2 dark:bg-background/50 bg-white/70 backdrop-blur-sm border dark:border-white/10 border-gray-200 rounded-l-full p-3 hover:bg-primary/20 transition-all"
              aria-label="Next testimonial"
            >
              <ChevronRight className="h-5 w-5 dark:text-white text-gray-800" />
            </button>
          </div>
            ...
            {testimonials.map((_, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all ${
                  currentIndex === index 
                    ? "bg-primary w-6" 
                    : "dark:bg-white/30 bg-gray-300 dark:hover:bg-white/50 hover:bg-gray-400"
                }`}
